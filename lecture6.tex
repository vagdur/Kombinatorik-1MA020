...
\begin{example}
  ...
  \begin{marginfigure}
    \includegraphics{graphics/checkerboard.png}
    \caption{Ett sätt att göra detta då $n=8$.}
  \end{marginfigure}
  ...
  \begin{marginfigure}
    \includegraphics{graphics/checkerboard_2.png}
    \caption{De två sätten att göra det på då $n=2$.}
  \end{marginfigure}
...
Vi har lärt oss att motsvarigheten till en faltning på den kombinatoriska sidan är en produkt av genererande funktioner. Vad händer om vi i stället tar \emph{exponentiella} genererande funktioner? Situationen blir lite mer komplicerad.\sidenote[][-0.7cm]{Det verkar som att både kursboken och förra årets föreläsningsanteckningar sveper denna komplikation under mattan, på ett sätt som jag upplever som väldigt förvirrande. Det tog mig minst en timme att lista ut varför vad de gör inte är direkt felaktigt.
\begin{center}\includegraphics[width=0.2\textwidth]{graphics/warning_sign.png}\end{center}}
...
På ett fullkomligt analogt sätt, genom att använda resultatet för ordinära genererande funktioner, fast med fler index, kan vi bevisa följande resultat:
\begin{lemma}
...
\begin{xca}
  Hur många strängar finns det ur alfabetet $\{a,b,c\}$, om vi kräver att
  \begin{itemize}
    ...
    \item och antalet $a$ är delbart med tre?
  \end{itemize}
\end{xca}
...
Låt $y_1 = x_1 + 7$, så att den nya begränsningen blir $y_1 \geq 0$. Vi får då \begin{align*}
x_1 + x_2 + x_3 &= 578 \\
y_1 - 7 + x_2 + x_3 &= 578 \\
y_1 + x_2 + x_3 &= 585
\end{align*}Låt sedan $a_k^1$ vara antalet lösningar till $y_1 = k$ med icke-negativt heltal $y_1$, $a_k^2$ vara antalet lösningar till $x_2 = k$ med icke-negativt jämna heltal $x_2$, och $a_k^3$ vara antalet lösningar till $x_3 = k$ med icke-negativa blåa/röda jämna och gula/gröna/lila udda heltal $x_3$.

Vi får alltså, då $y \geq 0$ att $a_1 = \left(\frac{1}{1-x}\right)$, den genererande funktionen för heltalen. $a_2$ är bara de jämna talen, som vi redan vet har den genererande funktionen $a_2 = \frac{1}{1-x^2}$. För $a_3$ vill vi ha $2$ på de jämna positionerna och $3$ på de udda. Detta fås av $\frac{2}{1-x^2}+\frac{3x}{1-x^2}=\frac{2+3x}{1-x^2}$

Vi får nu att $$F_a(x) = \left(\frac{1}{1-x}\right)\left(\frac{1}{1-x^2}\right)\left(\frac{2+3x}{1-x^2}\right)$$

Stoppa in i CAS ger att antalet lösningar för $k=585$ är $215355$
\end{solution}
\begin{tips}
För problem på denna form $\sum a_i = k$ gäller det bara att vara noggran så att man definierar sina $a_i$ korrekt. Har man en ``negativ begränsning'' som med $a_1 \geq -7$ i detta fall är det enklast att göra variabelbyte och justera ens $k$.
\end{tips}
\begin{solution}
    
$EG_b(x) = \sum_{k=0}^{\infty}b_k\frac{x^k}{k!} = \sum_{k=0}^{\infty}ka_k\frac{x^k}{k!} = \sum_{k=0}^{\infty}a_k\frac{x^k}{(k-1)!} = x\sum_{k=0}^{\infty}a_k\frac{x^{k-1}}{(k-1)!} = x\frac{d}{dx}\left(\sum_{k=0}^{\infty}a_k\frac{x^k}{k!}\right)=xF'$

\end{solution}
\begin{tips}
Kom ihåg att man med exponentiella genererande funktioner lätt kan plocka på sig derivator.
\end{tips}
\begin{solution}
Vi vill alltså först och främst se varför: $G(A(a)) = \frac{1}{x}\left(G(a) - T_0(a)\right)$.

Vi tar och definerar vad A(a) och ${t_0}$ är i kontexten.
  \begin{equation}
    G(A(a)_k) = \sum_{i=0}^{k}a_{i+1}x^i
  \end{equation}
...
Vi kan nu skriva om detta som:
  ...
  \begin{equation}
      G(A(a)) = \sum_{i=0}^{k}a_{i+1}x^i = \frac{1}{x} \sum_{i=0}^{k}a_{i+1}x^{i+1}
  \end{equation}

  \begin{equation}
      \frac{1}{x} \sum_{i=0}^{k}a_{i+1}x^{i+1} = \frac{1}{x} \sum_{i=1}^{k+1}a_{i}x^{i} = \frac{1}{x} (\sum_{i=0}^{k+1}a_{i}x^{i} -a_0)
  \end{equation}

Jämför det med det vi vill ha vilket är:

  \begin{equation}
      \frac{1}{x}\left(G(a) - T_0(a)\right) = \frac{1}{x} (\sum_{i=0}^{k}a_ix^i - a_0)
  \end{equation}

Eftersom att k går imot oändligheten så är det dock löst då det blir
  \begin{equation}
    \frac{1}{x} (\sum_{i=0}^{\infty}a_ix^i - a_0)
  \end{equation}

För både k+1 och k.

Vi kollar nu på diagrammet

    \begin{center}
      \includegraphics[width=0.4\textwidth]{graphics/abstract_exercise_comm_diag.png}
    \end{center}

Ett sätt att visa att det är sant är att ta Kompositionerna $EG \circ D$ och $A \circ EG$ och se om de ger samma sak från F.

\begin{equation}
    EG(A(a)_k) = \sum_{i=0}^{\infty}\frac{a_{i+1}x^i}{i!}
\end{equation}

\begin{equation}
    EG(a_k) = \sum_{i=0}^{k}\frac{a_ix^i}{i!}
\end{equation}

\begin{equation}
    D(EG(a_k)) = \frac{d}{dx} \sum_{i=0}^{k}\frac{a_ix^i}{i!} = \sum_{i=1}^{k}\frac{a_ix^{i-1}}{(i-1)!}
\end{equation}
  
\begin{equation}
    \sum_{i=1}^{k}\frac{a_ix^{i-1}}{(i-1)!} = \sum_{i=0}^{k}\frac{a_{i+1}x^{i}}{i!}
\end{equation}

Alltså är 

\begin{equation}
    D(EG(a_k)) = \sum_{i=0}^{k}\frac{a_{i+1}x^{i}}{i!} =  EG(A(a)_k)
\end{equation}
...
Om vi sätter $a = EG(a_k)$ får vi då

\begin{equation}
    EG(a_k) = \overrightarrow{1} - EG(A(a_k)) - EG(A(A(a_k)))
\end{equation}

Genom att använda det vi tidigare vet får vi då:

\begin{equation}
    EG(a_k) = \overrightarrow{1} - D(EG(a_k)) - D^2(EG(a_k))
\end{equation}

Om vi sätter $EG(a_k) = f $ får vi:

\begin{equation}
    f = \overrightarrow{1} - \frac{df}{dx} - \frac{d^2f}{dx^2}
\end{equation}

Nu ska vi använda fint matematiskt språk här, ``yeetar" vi in detta i vår favorit CAD Wolfram Alpha får vi:

\begin{equation}
    EG(a_k) = c_1 e^{-\frac{x}{2}} sin (\frac{\sqrt{3}x}{2})+c_2e^{-\frac{x}{2}}cos(\frac{\sqrt{3}x}{2}) + 1
\end{equation}
\end{solution}
