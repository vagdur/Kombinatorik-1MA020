...
\begin{definition}
    \emph{Väntevärdet} av en slumpvariabel $X$ som tar värden i $\R$\sidenote[][]{Samma definition hade fungerat precis lika väl över de komplexa talen. För den mer matematiskt sinnade kan det nog vara intressant att fundera över vad vi verkligen behöver anta om rummet vår slumpvariabel tar värden i för att väntevärdet skall bli väldefinierat.} ges av\sidenote[][-2.5cm]{Notera att detta är en summa över \emph{alla värden som $X$ kan tänkas ta} -- eftersom vi antagit att $\Omega$ är ändligt eller uppräkneligt så kommer detta vara en summa över ändligt eller uppräkneligt många summander, vilket är okej.
