...
\begin{xca}
    Bevisa att $\binom{n}{k}$ och $\stirlingPart{n}{n-k}$ är polynom i $n$ om vi håller $k$ fixt.\sidenote[][-1cm]{Ledtråd för partitionstalen: Hur många av elementen kan hamna i en hög där de inte är enda elementet i högen? Vi bevisade i föreläsning fyra att $\stirlingCycle{n}{n-3}$ är et polynom i $n$, kan ett liknande bevis lösa detta problem?}
\end{xca}

\begin{xca}
    Ge ett kombinatoriskt bevis för följande likhet, där vi antar att $x$ är ett heltal:
    $$\sum_{k = 1}^{n} \stirlingPart{n}{k}x(x-1)\ldots(x-k+1) = x^n.$$
\end{xca}
