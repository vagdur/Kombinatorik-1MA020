...
\begin{fullwidth}
    \begin{tabularx}{\linewidth}{l|ccc}
      ...
      Osärskiljbara lådor & \specialcell{Mängdpartition av $X$\\ i $\leq x$ delar \\$\sum_{k=1}^{x} \stirlingPart{n}{k}$} & \specialcell{Mängdpartition av $X$\\ i $\leq x$ delar av storlek $1$\\$1$ om $n \leq x$, $0$ annars} & \specialcell{Mängdpartition av $N$\\i $x$ delar\\$\stirlingPart{n}{x}$} \\
